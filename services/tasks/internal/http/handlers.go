...
// NewMux принимает опциональные *cache.Cache и *rabbit.Producer.
// Оба могут быть nil — сервис продолжит работу без кэша и без событий.
func NewMux(log *zap.Logger, client *authclient.GRPCClient, repo repository.TaskRepository, c *cache.Cache, producer *rabbit.Producer) http.Handler {
	...
	mux.HandleFunc("/v1/tasks", func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodPost:
			...
			// Задача уже создана в БД — публикуем событие.
			// Если RabbitMQ недоступен — логируем ошибку, но не падаем.
			if producer != nil {
...
func Run(log *zap.Logger) error {
	...
	if producer == nil {
		log.Warn("rabbitmq unavailable, running without events",
	...
	return http.ListenAndServe(":"+port, NewMux(log, authClient, repo, c, producer))
}
